...
public class GamePanel extends JPanel implements ActionListener, java.awt.event.ActionListener{
    ...
    static final int GAME_UNITES = (SCREEN_WIDTH*SCREEN_HEIGTH)/UNIT_SIZE;
    static final int DELAY = 100;
    final int x[] = new int[GAME_UNITES];// holds the x coordinate of the snakes body part including head.
    final int y[] = new int[GAME_UNITES]; // holds all the y coordinates.
    int bodyParts = 6;
    int applesEaten;
    int appleX;
    int appleY;
    ...
    public void draw(Graphics g){
        if(running){
            ...
            g.setColor(Color.red);
            g.setFont(new Font("Video Game Font", Font.BOLD, 40));
    ...
    public void checkCollision(){
        ...
        // this checks if head touches left boreder.
        if(x[0] < 0){
            running = false;
        }
        // if head touches right boreder.
        if(x[0] > SCREEN_WIDTH){
            running = false;
        }
        // if head touches top boreder.
        if(y[0] < 0){
            running = false;
        }
        // if head touches bottom boreder.
